...
function FieldEditor({ field, onUpdate, onDelete }: any) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-gaming-surface border border-gaming-border rounded-lg p-4 group">
        ...
        {fields.map((field: any) => (
          <div key={field.id}>
            ...
            {field.type === 'text' || field.type === 'email' || field.type === 'number' || field.type === 'date' ? (
              <input type={field.type} placeholder={field.placeholder} className="w-full px-3 py-2 bg-gaming-surface border border-gaming-border rounded-lg text-white text-sm focus:border-neon-blue focus:outline-none" disabled />
            ) : field.type === 'textarea' ? (
              <textarea placeholder={field.placeholder} rows={3} className="w-full px-3 py-2 bg-gaming-surface border border-gaming-border rounded-lg text-white text-sm focus:border-neon-blue focus:outline-none resize-none" disabled />
            ) : field.type === 'select' ? (
              <select className="w-full px-3 py-2 bg-gaming-surface border border-gaming-border rounded-lg text-white text-sm" disabled><option>{field.placeholder || 'Sélectionner...'}</option></select>
            ) : field.type === 'checkbox' ? (
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      {activeTab === 'list' && (
        <div className="space-y-4">
          {templates.map((form: any, i: number) => (
            <motion.div key={form.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-gaming-card border border-gaming-border rounded-xl p-5 hover:border-cyan-400/30 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-gaming-card border border-gaming-border rounded-xl p-5 space-y-4">
              <input value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="Nom du formulaire" className="w-full px-4 py-3 bg-gaming-surface border border-gaming-border rounded-lg text-white text-lg font-medium focus:border-neon-blue focus:outline-none" />
              <input value={formDesc} onChange={(e) => setFormDesc(e.target.value)} placeholder="Description (optionnelle)" className="w-full px-4 py-2 bg-gaming-surface border border-gaming-border rounded-lg text-white text-sm focus:border-neon-blue focus:outline-none" />
            </div>
            ...
            <div className="bg-gaming-card border border-gaming-border rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(fieldTypeLabels).map(([type, label]) => {
                  ...
                  return (
                    <button key={type} onClick={() => addField(type)} className="flex items-center gap-2 p-3 bg-gaming-surface border border-gaming-border rounded-lg text-gray-300 hover:text-white hover:border-neon-blue/30 transition-all text-sm">
      ...
      >
        {responses.map((resp: any) => (
          <div key={resp.id} className="bg-gaming-surface border border-gaming-border rounded-lg p-4 mb-3">
