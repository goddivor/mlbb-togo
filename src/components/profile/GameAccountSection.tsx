...
import { ChevronRight, Crown, EyeOff, Flame, Gamepad2, History, MapPin, Star, Swords, Trophy } from 'lucide-react';
...
import { GAME_STATS_ENABLED } from '@/config/gameStats';
  ...
  const tiles = [
    {
      ...
      value: stats.total,
      hint: t('dashboard.quick.record', { wins: stats.wins, losses: stats.losses }),
    ...
    },
    {
      ...
      value: `${stats.winRate}%`,
      hint: t('gameAccount.gameTime', { n: Math.round(stats.gameTime || 0) }),
    ...
    },
    {
      ...
      hint: t('gameAccount.mvpHint', { rate: mvpRate }),
      accent: 'gold' as const,
    },
    {
      key: 'score',
      icon: <Star size={16} />,
      label: t('gameAccount.avgScore'),
      value: stats.avgScore,
      hint: t('gameAccount.bestStreakHint', { n: stats.winStreak }),
...
function RankTile({ label, rank, extra, gold }: { label: string; rank: string; extra?: ReactNode; gold?: boolean }) {
  return (
    <div className="flex items-center gap-2.5">
      {hasRankBadge(rank) ? <RankBadge rank={rank} size={36} /> : <Trophy size={18} className="text-accent-gold" />}
      <div className="leading-tight">
        <p className="text-[11px] text-ink-3">{label}</p>
        <p className={cn('text-sm font-semibold', gold ? 'text-accent-gold' : 'text-ink-1')}>
          {rank}
          {extra}
        </p>
      </div>
...
/** Base profile still served by Moonton (getBaseInfo): identity, level, ranks, country. */
export function GameIdentity({ profile }: { profile: any }) {
  const t = useT();
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex min-w-0 items-center gap-3">
        <RankFrame name={name} src={profile.avatar ? avatarSrc(profile.avatar, 160) : null} rank={profile.rank} size={56} />
        <div className="min-w-0">
          <p className="truncate font-display text-lg font-bold leading-tight text-ink-1">{name}</p>
          <p className="mt-1 flex flex-wrap items-center gap-2 text-xs text-ink-2">
            ...
            {profile.country && (
              <span className="inline-flex items-center gap-1 num" title={t('gameAccount.country')}>
                <MapPin size={12} className="text-ink-3" /> {profile.country}
              </span>
            )}
          </p>
        </div>
      </div>
      {(profile.rank || profile.peakRank) && (
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
          {profile.rank && (
            <RankTile
              ...
              rank={profile.rank}
              extra={
                profile.rankLevel != null && <span className="ml-1.5 text-xs font-semibold text-ink-3 num">{profile.rankLevel} pts</span>
              }
            />
          )}
          {profile.peakRank && <RankTile label={t('dashboard.peakRank')} rank={profile.peakRank} gold />}
        </div>
      )}
    </div>
  );
...
/**
 * "Compte de jeu" section of a player profile, from our cached data (never
 * Moonton directly). Moonton only serves the base profile since MLBB Academy
 * closed, so the identity block is always shown and the detailed stats blocks
 * (career, seasons, heroes, match history, scoreboard) only render when they
 * hold cached data, or when GAME_STATS_ENABLED is switched back on.
 */
export default function GameAccountSection({
  userId,
  action,
  onReconnect,
  refreshKey,
  ...
  userId: string;
  /** Extra header action (the owner's sync button on /profile). */
  action?: ReactNode;
  /** Opens the link modal in place instead of navigating to /profile. */
  onReconnect?: () => void;
  /** Change it to refetch (e.g. after a manual sync). */
  refreshKey?: unknown;
}) {
  const t = useT();
  ...
  // Without a live stats source, only query the history when matches are cached.
  const enabled = !!game?.visible && !!game?.linked && (GAME_STATS_ENABLED || (game?.matchesCount ?? 0) > 0);
  useEffect(() => {
    if (!enabled) return;
    setListLoading(true);
    loadMatches(1)
  ...
  const hasCached =
    !!game.stats || (game.frequentHeroes || []).length > 0 || (game.seasonStats || []).length > 0 || (game.matchesCount ?? 0) > 0;
  const showStats = GAME_STATS_ENABLED || hasCached;
  const showCareer = GAME_STATS_ENABLED || !!game.stats;
  const showHeroes = GAME_STATS_ENABLED ? heroes.length > 0 || season != null : heroes.length > 0;
  const showHistory = enabled;
  ...
  return (
    <Card>
      ...
      />
      {action && showStats && seasons.length > 0 && <div className="-mt-2 mb-4 flex justify-end">{action}</div>}

      <GameIdentity profile={game.profile} />

      <GameSyncNotice sync={game.sync} isOwner={game.isOwner} onReconnect={onReconnect} className="mt-4" />

      {/* Career */}
      {showCareer && (
        <>
          <p className="eyebrow mb-3 mt-6">{t('gameAccount.career')}</p>
          {game.stats ? (
      ...
      {showHeroes && (
        <>
          <p className="eyebrow mb-3 mt-6">
            ...
            {season != null && (
              <span className="ml-2 normal-case tracking-normal text-ink-3">{t('gameAccount.season', { n: season })}</span>
            )}
          </p>
        ...
        <>
          ...
          {listLoading ? (
            ...
            <div className="flex flex-col items-center gap-2 rounded border border-dashed border-line-subtle px-4 py-8 text-center">
              ...
              <p className="max-w-sm text-sm text-ink-2">{t('gameAccount.historyEmpty')}</p>
            </div>
          ) : (
