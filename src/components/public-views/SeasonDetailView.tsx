...
export function SeasonDetailView() {
  ...
  const columns: DataColumn<StandingRow>[] = [
    { key: 'rank', header: '#', width: 'w-12', render: (r) => <span className="font-display font-bold text-ink-3">{r.rank}</span> },
    {
      ...
      render: (r) => (
        <Link href={`/dashboard/teams/${r.team.id}`} className="inline-flex items-center gap-2.5 font-semibold text-ink-1 hover:text-primary">
            ...
            <SeasonHero
              ...
              actions={
                season.status === 'closed' && season.slug ? (
                  <>
                    <Link href={`${base}/awards?season=${season.slug}`}>
