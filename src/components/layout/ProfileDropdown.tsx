...
/** TailAdmin "My Profile" person glyph. */
...
/** TailAdmin "Account Settings" gear glyph. */
...
/** External-link glyph for the admin "back to site" entry. */
function ExternalIcon() {
  return (
    ...
    >
      <path
        d="M17.4375 11.7906C17.0250 11.7906 16.6812 12.1344 16.6812 12.5469V17.4906C16.6812 18.2125 16.0968 18.7969 15.375 18.7969H4.53748C3.81561 18.7969 3.23123 18.2125 3.23123 17.4906V6.65312C3.23123 5.93125 3.81561 5.34687 4.53748 5.34687H9.48123C9.89373 5.34687 10.2375 5.00312 10.2375 4.59062C10.2375 4.17812 9.89373 3.83437 9.48123 3.83437H4.53748C2.98123 3.83437 1.71873 5.09687 1.71873 6.65312V17.4906C1.71873 19.0469 2.98123 20.3094 4.53748 20.3094H15.375C16.9312 20.3094 18.1937 19.0469 18.1937 17.4906V12.5469C18.1937 12.1344 17.85 11.7906 17.4375 11.7906Z"
        fill=""
      />
      <path
        d="M19.9375 1.65625H14.4375C14.025 1.65625 13.6813 2 13.6813 2.4125C13.6813 2.825 14.025 3.16875 14.4375 3.16875H17.7031L9.24065 11.6656C8.9469 11.9594 8.9469 12.4406 9.24065 12.7344C9.3781 12.8719 9.58436 12.9406 9.75623 12.9406C9.9281 12.9406 10.1344 12.8719 10.2719 12.7344L18.7344 4.2375V7.50312C18.7344 7.91562 19.0781 8.25937 19.4906 8.25937C19.9031 8.25937 20.2469 7.91562 20.2469 7.50312V2.00312C20.6937 1.99999 20.35 1.65625 19.9375 1.65625Z"
        fill=""
      />
    </svg>
  );
...
/**
 * TailAdmin profile menu (see partials/header.html): avatar + name + role +
 * chevron trigger opening a dropdown with navigation links (each with its
 * exact TailAdmin SVG) and a Log Out action carrying the TailAdmin logout
 * glyph. The parent header supplies the links and the concrete logout logic,
 * so the same component serves both the player and the admin shells. Closes on
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="group flex items-center gap-2.5 rounded-md py-1 pl-1 pr-1.5 transition-colors duration-fast hover:bg-surface-2/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 sm:pr-2"
        aria-label="Menu profil"
        aria-expanded={open}
      >
        <span className="h-8 w-8 shrink-0 overflow-hidden rounded-full ring-2 ring-primary/50 ring-offset-2 ring-offset-surface-1">
          {avatarUrl ? (
            ...
            <span className="flex h-full w-full items-center justify-center bg-gradient-to-br from-accent-cyan to-accent-violet font-display text-sm font-bold text-on-primary">
              {initial}
            </span>
          )}
        </span>
        ...
        <span className="hidden min-w-0 text-left lg:block">
          <span className="block max-w-[10rem] truncate text-[13px] font-semibold leading-tight text-ink-1">{name}</span>
          {subtitle && <span className="block truncate text-[11px] leading-tight text-ink-3">{subtitle}</span>}
        </span>

        <svg
          className={`hidden fill-current text-ink-3 transition-transform duration-fast sm:block ${open ? 'rotate-180' : ''}`}
          width="10"
          height="7"
          viewBox="0 0 12 8"
          fill="none"
      ...
      {open && (
        <div className="header-menu absolute right-0 mt-2.5 flex w-60 flex-col overflow-hidden">
          ...
          <ul className="flex flex-col gap-0.5 border-b border-line-subtle p-1.5">
            {links.map((link) => {
              ...
              return (
                <li key={link.href}>
                  <Link
                    ...
                    className="flex items-center gap-3 rounded-md px-2.5 py-2 text-sm font-medium text-ink-2 transition-colors duration-fast hover:bg-surface-2 hover:text-ink-1 [&_svg]:h-[18px] [&_svg]:w-[18px]"
                  >
                    <Icon />
                    {link.label}
                  </Link>
          ...
          <div className="p-1.5">
            <button
              ...
              className="flex w-full items-center gap-3 rounded-md px-2.5 py-2 text-sm font-medium text-ink-2 transition-colors duration-fast hover:bg-accent-red/10 hover:text-accent-red [&_svg]:h-[18px] [&_svg]:w-[18px]"
            >
              <svg
                className="fill-current"
                width="22"
